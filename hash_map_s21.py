...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        for linked_list in self._buckets: # for each linked_list in the da buckets ( its a list of linked lists)
        # calculate index for each key for the temporary hashMap
            if linked_list != None: # exception if a linked_list is not in a bucket
                cur = linked_list # first value in LL
                while cur != None: # traverse through the LL until the end
                    val = cur.value # found the value
                    key = cur.key # found the key
                    # and put the key and value in the temporary hashMap
                    hashing_index = temp._hash_function(key) % temp.capacity # hash the key and mod to ensure its within capacity
                    temp[hashing_index].add_front(key, val) # Use the hashing index to get the LL and add the key value to the front of the LL
                    cur = cur.next # move on
        
        # point the existing hashMap to the temporary hashMap and change capacity as well
        self._buckets = temp._buckets # set the temp bucket to the current hash map bucket
        self.capacity = temp.capacity # set the temp capacity to the current hash map capacity
